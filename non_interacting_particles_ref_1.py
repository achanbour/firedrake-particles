...
def move_particles_in_ref_space(pmesh, mesh, v_fn, dt, T, t=0.0):
    ...
    while t < T:
        ...
        while True:
            ...
            # -- Process failed particles in Phase 1 -- 
            ...
            if len(failed_global) > 0:
                ...
                dt_left[failed_global] -= t_cross_final
                crossed_edges[failed_global] = crossed_edges_final
                ref_coords_register[failed_global] = ref_coords_final
...
def detect_crossings(bary_old, bary_new, dt_left, edges, tol=1e-12):
    """Detect arbitrary crossings based on barycentric coordinates."""
...
def move_failed_particles_to_facet(
        failed_global, t_cross0, crossed_edges0, ref_coords_fn, invJ_vom, v_fn, FS_vom, ref_cell, ref_cell_edges):
    ...
    dt_curr = t_cross0
    still_failed_local = np.arange(n_failed) # track local indices of still failed particles
    ...
    # Declare end results arrays for failed subset only
    t_cross_final = np.zeros(n_failed)
    crossed_edges_final = np.full(n_failed, None, dtype=object)
    ref_coords_final = np.zeros((n_failed, FS_vom.mesh().geometric_dimension))

    # Loop until every failed particle finds a unique crossing facet
    inner_loop_iter = 0
    while len(still_failed_local) > 0:
        inner_loop_iter += 1
        ...
        dt_step_fn.dat.data[failed_global[still_failed_local]] = dt_curr

        ref_step_fn = update_ref_pos(ref_coords_fn, invJ_vom, v_fn, dt_step_fn)

        bary_old = ref_cell.compute_barycentric_coordinates(
            np.array(ref_coords_fn.dat.data_ro)[failed_global[still_failed_local]]
        )
        bary_new = ref_cell.compute_barycentric_coordinates(
            np.array(ref_step_fn.dat.data_ro)[failed_global[still_failed_local]]
        )
        ...
        passed_local_idx = still_failed_local[passed_mask]
        t_cross_final[passed_local_idx] = dt_curr[passed_mask]
        crossed_edges_final[passed_local_idx] = edge_ids_curr[passed_mask]
        ref_coords_final[passed_local_idx] = ref_step_fn.dat.data_ro[failed_global[passed_local_idx]]

        # Update state for next iteration
        still_failed_local = still_failed_local[~passed_mask]
